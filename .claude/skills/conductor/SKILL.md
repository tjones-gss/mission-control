...
| 2 Build         | Per task: `test-writer` ║ `worker` → `validator`(scope=task). On fail: append to `attempts/<task>.md`, increment `task_iters[id]`, re-spawn worker. Max 5 iters → dispatch `planner`(mode=split). If split lineage already has `splits >= 2`, escalate instead of re-splitting. **Validator collapse:** when every Phase-2 task is independent (`blockedBy: []`) and the spec's `acceptance_commands` are suite-level runs that cover all tasks, per-task validators MAY be deferred to a single slice validator at the start of Phase 3. Record the deviation as `{"event":"per_task_validators_skipped","reason":"<why>"}` in `events.jsonl`. If the slice validator fails, the orchestrator MUST then dispatch per-task validators to localize the failure before re-spawning workers — collapse is an optimization, not a substitute for per-task feedback when something breaks. |
...
- **v0.2 (this version)**
  ...
  - Phase-2 validator collapse authorized when tasks are independent and share the spec's suite-level `acceptance_commands`. Deviation logged to `events.jsonl`; failure on the slice validator falls back to per-task validators before re-spawning workers.
