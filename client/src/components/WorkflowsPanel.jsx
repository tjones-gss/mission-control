...
function stepSummary(step) {
  ...
  if (step.type === 'agent') return `${step.agentType || '?'}: ${(step.prompt || '').slice(0, 40)}${(step.prompt || '').length > 40 ? '…' : ''}`
  if (step.type === 'instruction') return (step.text || '').slice(0, 50) + ((step.text || '').length > 50 ? '…' : '')
...
export function WorkflowsPanel({ workflows, loading, refetch, skills }) {
  ...
  async function save() {
    ...
    try {
      if (isNew) {
        ...
        if (res.status === 409) { setError('A workflow with that name already exists.'); return }
        if (!res.ok) { setError('Failed to create workflow.'); return }
  ...
  async function doExport(overwrite = false) {
    ...
    try {
      ...
      if (res.status === 409) { setExportConflict(true); return }
  ...
  async function deleteWorkflow(name) {
    try {
      ...
      if (selectedName === name) { setSelectedName(null); setDraft(null); setIsNew(false) }
      await refetch()
    } catch {
    ...
    <div className="flex flex-1 overflow-hidden h-full">
          ...
          {list.map(wf => (
            ...
            >
              <div className="min-w-0">
                ...
                <div className="text-xs text-gray-600">{wf.steps?.length || 0} step{wf.steps?.length !== 1 ? 's' : ''}</div>
              </div>
              <button
                className="ml-1 text-gray-700 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all shrink-0"
                onClick={e => { e.stopPropagation(); setDeleteConfirm(wf.name) }}
          ...
          <div className="p-4 flex flex-col gap-4 min-h-full">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {draft.steps.map((step, i) => (
                ...
                >
                  ...
                  <span className={`text-xs px-1.5 py-0.5 rounded shrink-0 ${STEP_BADGE[step.type] || 'bg-gray-800 text-gray-400'}`}>
                    {step.type}
                  </span>
                  <span className="text-xs text-gray-300 truncate flex-1">{stepSummary(step)}</span>
                  <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => moveStep(i, -1)} disabled={i === 0} className="text-gray-600 hover:text-gray-300 disabled:opacity-30 transition-colors"><ChevronUp size={12} /></button>
                    <button onClick={() => moveStep(i, 1)} disabled={i === draft.steps.length - 1} className="text-gray-600 hover:text-gray-300 disabled:opacity-30 transition-colors"><ChevronDown size={12} /></button>
                    <button onClick={() => setEditingStep({ step, index: i })} className="text-gray-600 hover:text-gray-300 transition-colors"><Pencil size={11} /></button>
                    <button onClick={() => removeStep(i)} className="text-gray-600 hover:text-red-400 transition-colors"><X size={12} /></button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-gray-800 pt-3 mt-auto">
              ...
              {exportSuccess && !exportConflict && (
                <div className="text-xs text-green-400 mb-3">Exported as <code className="bg-gray-800 px-1 rounded">/{exportName}</code></div>
              )}
              {exportConflict && (
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs text-yellow-400">Skill <code className="bg-gray-800 px-1 rounded">/{exportName}</code> already exists.</span>
                  <button onClick={() => doExport(true)} className="text-xs bg-yellow-700 hover:bg-yellow-600 text-white px-2 py-0.5 rounded transition-colors">Overwrite</button>
                  <button onClick={() => setExportConflict(false)} className="text-xs text-gray-500 hover:text-gray-300 transition-colors">Cancel</button>
                </div>
              )}
              <div className="flex items-center gap-2">
                <button
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-gray-900 border border-gray-700 rounded-lg p-5 shadow-xl max-w-xs" onClick={e => e.stopPropagation()}>
            <p className="text-sm text-gray-200 mb-1">Delete <span className="font-semibold">{deleteConfirm}</span>?</p>
            <p className="text-xs text-gray-500 mb-4">This cannot be undone.</p>
            <div className="flex gap-2">
              <button onClick={() => deleteWorkflow(deleteConfirm)} className="px-3 py-1.5 text-xs bg-red-700 hover:bg-red-600 text-white rounded transition-colors">Delete</button>
              <button onClick={() => setDeleteConfirm(null)} className="px-3 py-1.5 text-xs text-gray-400 hover:text-gray-200 transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
