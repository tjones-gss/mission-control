  ...
  return (
    <div className="h-full flex flex-col overflow-hidden relative">
      <div ref={scrollRef} onScroll={handleScroll} className="flex-1 overflow-y-auto px-3 py-2">
        ...
        {events.length === 0 && !loading && <div className="text-xs text-gray-700 p-4">No events</div>}
        {events.map((evt, i) => (
          <div key={i} className="flex items-start gap-2 py-1.5 border-b border-gray-900/50">
            ...
            <div className={`text-[10px] font-semibold uppercase tracking-wider shrink-0 w-14 ${evt.color}`}>
              {evt.label}
            </div>
