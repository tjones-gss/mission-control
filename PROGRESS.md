...
- 2026-06-25: L2 verify (ADOPTABLE) + Windows test:cockpit hardening. Audited all 3 L2 criteria: L2-a (App.test.jsx first-run hero gating + WelcomeHero) and L2-b (rails-installer + routes/rails: one-click adopt lands pure-Node `*.mjs` hooks) were already proven; L2-c (`ci.yml` server+client `--coverage` gates with real floors + gated Fleet/Playwright e2e job) proven and now GUARDED by a new `tests/contracts/ci-gates.contract.test.js` (+4) so the gates can't silently regress (`92caec5`). Wrote GOALS_L2.md. Also `ea441e0` — hardened the canonical `npm run test:cockpit` on Windows: `npx vitest run` raced the nested cmd.exe resolution ("'vitest' is not recognized"); fixed by adding `test: vitest run` to server/ and client/ package.json and delegating via `cd <ws> && npm test`. Canonical command now passes end-to-end: server 1365/0 (98 files), client 703/0 (67 files), lint clean. NOTE: rails fs tests can flake under full parallel execution on Windows (recursive adapter copy + rmSync under AV/file-lock) — pass in isolation, on re-run, and in CI; documented class, not a code defect.
