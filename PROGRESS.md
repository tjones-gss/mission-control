...
**I3 — Knowledge Graph.** Per §8, I3 is next (I1+I2 complete). New files per spec:
`lib/db/knowledge-graph.js` (new SQLite `nodes` + `edges` tables; node kinds
session/file/decision/outcome/commit/task; relations touched/decided/produced/
blocked/spawned), `GET /api/graph?node=` (2-hop neighbourhood), and a `GraphPanel`
inside `InspectPanel/` (force-layout mini-graph, "Show graph" button in session
detail). Derived cache — deleting cockpit.db must rebuild from ~/.claude + git.
NOTE: this is another SQLite schema bump (v5 → v6).
